...
import { getUserData, createDream } from "@/lib/api"
...
export default function AddGoalPage() {
  ...
  const handleStartGoal = async () => {
    ...
    try {
      ...
      await createDream(targetUserId, newGoal.title, newGoal.amount, newGoal.deadline, newGoal.daily_limit, newGoal.icon, newGoal.iconBg, newGoal.iconColor)
      
      // Saytni to'liq yangilash (yangi maqsadni ko'rish uchun)
  ...
  return (
    <div className={`min-h-[100dvh] bg-gradient-to-br from-[#F4FCF9] via-[#FFFFFF] to-[#E9F8F1] flex justify-center ${poppins.className}`}>
        <div className="w-full max-w-sm flex flex-col min-h-[100dvh] overflow-hidden relative">
          
          {step === 1 && (
            <div className="flex flex-col h-full animate-in fade-in overflow-hidden">
              <div className="p-5 overflow-y-auto flex-1 scrollbar-none pb-0 relative">
                <button onClick={() => router.push('/')} className="absolute top-5 left-5 w-10 h-10 flex items-center justify-center bg-gray-50 hover:bg-gray-100 rounded-full text-gray-600 transition-colors">
